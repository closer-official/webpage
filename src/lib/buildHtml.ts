...
${petPol.map((p) => `      <details class="pet-acc-item"><summary class="pet-acc-sum">${escapeHtml(p.title)}</summary><div class="pet-acc-body">${appendPaymentLogosAfterBrContent(p.body, escapeHtml)}</div></details>`).join('\n')}
  ...
  const cafe1BranchMenuHtml = (): string => {
    ...
    for (const it of items) {
      ...
      chunks.push(
        `<a href="${escapeHtml(it.menuUrl)}" target="_blank" rel="noopener noreferrer" class="c1-menu-branch-btn">${escapeHtml(it.label)}</a>`
      );
    }
    return chunks.join('\n        ');
  };
    ...
    tid === 'cafe_tea' || tid === 'cafe_1'
      ? content.sections
          .map((s, i) => {
            ...
            /** cafe_1: コンセプト直下・店主より直下のリード写真はヒーロー等と被りやすいので出さない */
...
${cafe1ShopLocationsHtml()}
      ...
      <div class="section-body"><h2 id="${s.id}-title">${escapeHtml(s.title)}</h2>${paras}</div>
            ...
            if (tid === 'pet_salon' && s.id === 'gallery' && content.catalogImages?.length) {
      ...
      <div class="section-body"><h2 id="${s.id}-title">${escapeHtml(s.title)}</h2>${wrapBrParagraphWithPaymentLogos(s.content, escapeHtml)}${mapEmbed}</div>
            ...
            if (tid === 'ramen_2' && s.id === 'menu' && content.catalogImages?.length) {
  ...
  const cafe1HeroBelowHtml =
    ...
    <div class="container c1-hero-below-inner">
      ...
      ${String(content.subheadline ?? '').trim() ? `<div class="c1-hero-below-lede">${renderProseParagraphsWithPaymentLogos(String(content.subheadline ?? ''), escapeHtml)}</div>` : ''}
    </div>
  </section>`
      : '';
...
(function(){
  ...
  const wikiSaunaMainHtmlClient =
    tid === 'wiki_sauna' ? buildWikiSaunaDeliverableMainHtmlClient(escapeHtml) : '';
