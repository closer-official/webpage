...
export function buildHtml(content, seo, templateId, genOptions = {}) {
  ...
  const heroImageUrl =
    ...
    (seo.ogImageUrl && seo.ogImageUrl.trim()) ||
...
<div class="builder-nav-overlay" aria-label="メニュー" id="builder-nav-overlay">
  ...
  function parsePriceYenForJsonLd(price) {
    if (!price) return undefined;
    const digits = String(price).replace(/[,¥￥\s]/g, '').replace(/[^\d]/g, '');
    return digits || undefined;
  }
  function buildCafeMenuJsonLd(rows) {
    if (!rows || !rows.length) return undefined;
    const sections = [];
    let cur = null;
    for (const row of rows) {
      const g = String(row.groupLabel || '').trim() || 'メニュー';
      if (!cur || cur.name !== g) {
        if (cur) sections.push(cur);
        cur = { name: g, items: [] };
      }
      const displayName = [row.name, row.badge].filter(Boolean).join(' ') || 'メニュー';
      const item = { '@type': 'MenuItem', name: displayName };
      if (row.description && String(row.description).trim()) item.description = String(row.description).trim();
      const p = parsePriceYenForJsonLd(row.price);
      if (p) item.offers = { '@type': 'Offer', price: p, priceCurrency: 'JPY' };
      cur.items.push(item);
    }
    if (cur) sections.push(cur);
    return {
  ...
  if (tid === 'gym_yoga') {
    ...
    if (content.footerPhone) lb.telephone = content.footerPhone;
    if (content.footerAddress) {
      lb.address = { '@type': 'PostalAddress', streetAddress: content.footerAddress, addressCountry: 'JP' };
    }
    if (seo.ogImageUrl) lb.image = seo.ogImageUrl;
    jsonLdGraphs.push(lb);
  }
  if (tid === 'cafe_1') {
    ...
    if (content.footerPhone) r.telephone = content.footerPhone;
    const street = String(meo.streetAddress || content.footerAddress || '').trim() || undefined;
    ...
    if (meo.servesCuisine) r.servesCuisine = meo.servesCuisine;
    if (meo.priceRange) r.priceRange = meo.priceRange;
    if (meo.openingHours && meo.openingHours.length) r.openingHours = meo.openingHours;
    const imgList = [seo.ogImageUrl, ...((content.heroSlides || []).map((u) => String(u || '').trim()).filter(Boolean))].filter(
      Boolean
    );
    if (imgList.length === 1) r.image = imgList[0];
    else if (imgList.length > 1) r.image = imgList;
    const menuLd = buildCafeMenuJsonLd(content.cafeMenuTextRows || []);
    if (menuLd) r.hasMenu = menuLd;
    jsonLdGraphs.push(r);
  }
  ...
  if (faqItems.length > 0) {
    jsonLdGraphs.push({
      '@context': 'https://schema.org',
      '@type': 'FAQPage',
      ...(canonicalTrim ? { url: canonicalTrim } : {}),
...
${petPol.map((p) => `      <details class="pet-acc-item"><summary class="pet-acc-sum">${escapeHtml(p.title)}</summary><div class="pet-acc-body">${escapeHtml(p.body).replace(/\n/g, '<br>')}</div></details>`).join('\n')}
  ...
  function cafe1MenuTextHtml() {
    const rows = content.cafeMenuTextRows || [];
    if (!rows.length) return '';
    let lastG = '\0';
    const parts = [];
    for (const row of rows) {
      const g = String(row.groupLabel || '').trim();
      if (g && g !== lastG) {
      ...
      ? sections
          .map((s, i) => {
            ...
            if (
              tid === 'cafe_1' &&
              s.id === 'menu' &&
              ((content.cafeBranchMenuItems || []).length > 0 || (content.cafeMenuTextRows || []).length > 0)
            ) {
              const branchGrid =
                (content.cafeBranchMenuItems || []).length > 0
...
${cafe1ShopLocationsHtml()}
      ...
      ${img}
      <div class="section-body"><h2 id="${s.id}-title" class="wo-sec-heading">${escapeHtml(s.title)}</h2><div class="wo-sec-prose"><p>${escapeHtml(s.content).replace(/\n/g, '</p><p>')}</p></div>${bubbleHtml}</div>
    </section>`;
            }
            if (s.id === 'hours') {
...
<label for="wo-nav-toggle" class="wo-nav-fab" aria-label="メニュー"><span></span><span></span><span></span></label>`
  ...
  let gymBlocksHtml = '';
  if (tid === 'gym_yoga') {
    function gymStatValueHtml(st, i) {
      const anim = gymStatAnimations[i];
      if (anim && typeof anim.end === 'number' && Number.isFinite(anim.end)) {
        return (
          '<span class="gym-stat-value gym-stat-count" data-target="' +
          anim.end +
          '" data-suffix="' +
          escapeHtml(anim.suffix || '') +
          '" data-duration="1500">0' +
          escapeHtml(anim.suffix || '') +
          '</span>'
        );
      }
    ...
    if (gymAudienceHooks.length > 0) {
      const audIntro =
        (content.gymAudienceIntro && String(content.gymAudienceIntro).trim()) ||
    ...
    <section class="gym-results-block"${gymSi} aria-labelledby="gym-results-title">
      ...
      <div class="gym-results-stats">${gymStats.map((st, i) => `<div class="gym-stat-item">${gymStatValueHtml(st, i)}<span class="gym-stat-label">${escapeHtml(st.label)}</span></div>`).join('')}</div>
    </section>`;
    }
    ...
    <section class="gym-results-block gym-ba-section"${gymSi} aria-labelledby="gym-slider-title">
      ...
      <div class="gym-results-slider" role="list">${gymCatalogImages
        .map((url, i) => {
          ...
          const alt = escapeHtml(content.siteName + 'のトレーニング実績イメージ ' + (i + 1));
          return (
            '<div class="gym-results-slide" role="listitem"><img src="' +
            escapeHtml(url) +
            '" alt="' +
            alt +
            '" loading="lazy" width="600" height="600"><p class="gym-results-slide-caption">' +
            cap +
            '</p>' +
            voice +
            '</div>'
    ...
    if (gymMenuCards.length > 0 || gymProgramSteps.length > 0 || gymMenuCompareRows.length > 0) {
      const menuLede =
        (content.gymMenuLede && String(content.gymMenuLede).trim()) ||
        '対面はフォームと追い込みを最優先。オンラインは場所を選ばず継続しやすく。下の表で違いを比較し、自分に合う方を選んでください。';
      const stepsHtml =
        gymProgramSteps.length > 0
          ? `<div class="gym-program-block"${gymSi}><h3 id="gym-program-flow" class="gym-program-heading">パーソナルの流れ（対面）</h3><ol class="gym-program-steps" aria-labelledby="gym-program-flow">${gymProgramSteps
              .map(
                (s, i) =>
                  `<li class="gym-program-step"${gymSi}><span class="gym-program-step-num" aria-hidden="true">${i + 1}</span><div><strong class="gym-program-step-title">${escapeHtml(s.title)}</strong><p>${escapeHtml(s.body)}</p></div></li>`
              )
              .join('')}</ol></div>`
          : '';
      const compareHtml =
        gymMenuCompareRows.length > 0
          ? `<div class="gym-compare-wrap"${gymSi}><h3 class="gym-compare-heading">対面とオンラインの違い</h3><div class="gym-compare-scroll"><table class="gym-compare-table"><thead><tr><th scope="col">項目</th><th scope="col">対面パーソナル</th><th scope="col">オンライン</th></tr></thead><tbody>${gymMenuCompareRows
              .map(
                (row) =>
                  `<tr><th scope="row">${escapeHtml(row.feature)}</th><td>${escapeHtml(row.onsite)}</td><td>${escapeHtml(row.online)}</td></tr>`
              )
...
(function(){
  ...
  const paymentSectionHtml = tid !== 'event' && tid !== 'cafe_1'
    ? `
